...
contract testRandom is Test {
    function testGenerateNumberRange() public {
        vm.roll(200);
        ...
        for (uint256 blockNumber; blockNumber < 200; blockNumber++) {
        ...
        uint256 minLimit = (100_000 / 50) * (80 / 100);
        uint256 maxLimit = (100_000 / 50) * (120 / 100);
    ...
    function min(uint256[] memory numbers) internal pure returns (uint256) {
        require(numbers.length > 0);
